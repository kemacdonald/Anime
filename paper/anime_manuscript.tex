...
models using the \texttt{rstanarm} R package {[}CITE package{]}. The
mixed-effects approach allowed us to model the nested structure in our
data by including random intercepts for each participant and item and a
random slope for each item. We used Bayesian methods in order to
...
different cue types. To communicate the uncertainty in our estimates, we
report the 95\% Highest Density Interval (HDI) of the posterior
distribution of both the group means and the difference in means. The
HDI provides a range of plausible parameter values given the data and
the model. All analysis code can be found in the online repository for
this project: \url{https://github.com/kemacdonald/anime}.
...
specified as: \(log(RT) \sim cue\_type + (1 + sub\_id | item)\).
...
(\(M_{name}\) = 876.35 ms), as compared to its lexicalized animal sound
(\(M_{onomatopeia}\) = 649.70 ms), and its vocalization
(\(M_{vocalization}\) = 720.38 ms). The difference between RTs for the
name and lexicalized animal sounds was -70.67 ms with a HDI from -212.86
...
Finally, the average difference in children's RT between the lexicalized
...
children processed animal names more efficiently compared to lexicalized
animal sounds but strong evidence of faster processing of lexicalized
...
RT model described above:
\(logit(accuracy) ~ cue\_type + (1 + sub\_id | item)\). The orange
...
animals as their names or lexicalized sounds if children have enough
...
behavior to identify familiar animals as their names or lexicalized
...
Figure 5 shows children's proportion looking to the target animal after
hearing a familiar or a novel animal vocalization over the same window
used in Experiment 1 (300 to 2500 ms after the onset of the
vocalization). Visual inspection of the results suggest that children
...
plausible values. Moreover, the novel animal vocalization and the
familiar animal vocalization were equally effective in guiding
children's attention to the target animal over the course of the trial
...
mean proportion looking falling well within the range of plausible
values. Moreover, there was strong evidence that children were less
...
Three findings emerged from the accuracy analysis, the first two
replicated key findings from Experiment 1: First, children oriented to a
...
vocalization. In addition, we also found that children did not show
evidence of retaining the link between a novel animal vocalization and a
novel animal. These results and their implications will be discussed in
...
Horst, et al., 2009) and emphasizes the theoretical importance of
disentangling processes of disambiguating reference, an in-the-moment
phenomenon, and word learning, which occurs over a longer timescale.
...
argues that onomatopoeic sounds might provide young children with
...
target objects, while only 1 in 30 American mothers did so. While our
results do not speak direclty to these theoretical issues, they do
suggest that onomatopoeic sounds function like words in that they are
capable of activating conceptual representations that drive children's
visual attention to seek the physical referent of the sound. However,
there was some evidence that children processed onomatopeic sounds less
...
that learning operates over a much longer timescale. Although
...
retention (Horst, et al., 2006, Bion et al., 2013).
...
biases (Bion, Borovsky, \& Fernald, 2013; Weisleder, Hurtado, \&
Fernald, in preparation; Yurovsky et al., 2012). Other studies have
