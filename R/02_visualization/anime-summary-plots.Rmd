...
e1_acc_plot <- d %>% 
  ...
  geom_line(data = ms_acc_e1, aes(group = 1), color = "darkred", size = 1,
            position = position_nudge(x = 0.3)) + 
  geom_pointrange(data = ms_acc_e1, aes(ymin = hdi_lower, ymax = hdi_upper), color = "darkred", 
...
e2_acc_plot <- d %>% 
  ...
  geom_violin(draw_quantiles = 0.5, trim = F, width = 0.5, size = 1, color = "grey30",
              adjust = 1, fill = NA) + 
  geom_jitter(width = 0.03, alpha = 0.5, shape = 21, color = "black") +
  geom_line(data = ms_acc_e2, aes(group = 1), color = "darkred", size = 1,
            position = position_nudge(x = 0.3)) + 
  geom_pointrange(data = ms_acc_e2, aes(ymin = hdi_lower, ymax = hdi_upper), color = "darkred", 
                  fill = "darkred", shape=21, size = 0.5,
                  position = position_nudge(x = 0.3)) + 
